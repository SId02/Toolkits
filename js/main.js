...
/*Countries Details using restcountries Api */
...
function initialize(countriesData) {
  ...
  countries.forEach(function (country) {
    return options += `<option value="${country.alpha3Code}">${country.name}</option>`;
  });
...
function displayCountryInfo(countryByAlpha3Code) {
  ...
  document.querySelector("#flag").alt = `Flag of ${countryData.name}`;
  document.querySelector("#capital").innerHTML = countryData.capital;
  document.querySelector("#dialing-code").innerHTML = `+${countryData.callingCodes[0]}`;
  document.querySelector("#population").innerHTML = countryData.population.toLocaleString("en-mr");
  document.querySelector("#currencies").innerHTML = countryData.currencies.filter(function (c) {
    return c.name;
  }).map(function (c) {
    return "".concat(c.name, " (").concat(c.code, ")  (").concat(c.symbol, ")");
  }).join(", ");
